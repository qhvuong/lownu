...
{ 
  ...
  double chi2, CC_chi2_e, CC_chi2_m, nue_chi2;
  ...
  for( int bx = 1; bx <= target_nue->GetNbinsX(); ++bx ) {
    double tgt = target_nue->GetBinContent(bx);
  ...
  for( int bx = 0; bx < nbins_Elep; bx++ ) {
    ...
    for( int by = 0; by < nbins_Elep; by++ ) {
      ...
      if(bx == by) {
        ...
        unc[bx+2*nbins_Elep][by+2*nbins_Elep] = nue_em_target->GetBinContent(bx+1);
      }
      ...
      cov[bx][by+2*nbins_Elep] = covmtr[bx][by+2*nbins_Elep] * CC_m_target->GetBinContent(bx+1) * nue_em_target->GetBinContent(by+1);
      ...
      cov[bx+nbins_Elep][by+2*nbins_Elep] = covmtr[bx+nbins_Elep][by+2*nbins_Elep] * CC_e_target->GetBinContent(bx+1) * nue_em_target->GetBinContent(by+1);

      cov[bx+2*nbins_Elep][by]              = covmtr[bx+2*nbins_Elep][by]              * nue_em_target->GetBinContent(bx+1) * CC_m_target->GetBinContent(by+1);
      cov[bx+2*nbins_Elep][by+nbins_Elep]   = covmtr[bx+2*nbins_Elep][by+nbins_Elep]   * nue_em_target->GetBinContent(bx+1) * CC_e_target->GetBinContent(by+1);
      cov[bx+2*nbins_Elep][by+2*nbins_Elep] = covmtr[bx+2*nbins_Elep][by+2*nbins_Elep] * nue_em_target->GetBinContent(bx+1) * nue_em_target->GetBinContent(by+1);
    }
  }
...
void TemplateFitter::Draw()
{
  ...
  h0->Draw("colz");
  g0->Draw("same C*");
  c0->SaveAs(Form("chi2Surface_1%d0_%d.png",cutNo,cutEv));
  TCanvas *c1 = new TCanvas("c1","",800,600);
  c1->SetLogz();
  h1->SetMaximum(1E6);
  h1->Draw("colz");
  g1->Draw("same C*");
  c1->SaveAs(Form("chi2Surface_1%d1_%d.png",cutNo,cutEv));
  TCanvas *c2 = new TCanvas("c2","",800,600);
  c2->SetLogz();
  h2->SetMaximum(1E6);
  h2->Draw("colz");
  g2->Draw("same C*");
  c2->SaveAs(Form("chi2Surface_1%d2_%d.png",cutNo,cutEv));

  TFile *out = new TFile(Form("chi2_1%d_%d.root",cutNo,cutEv),"RECREATE");
  h0->Write();
  h1->Write();
  h2->Write();
  out->Close();
