...
*/
  ...
  TH1D *nmEv0 = new TH1D("nmEv0","",200,0,40); //numu acceptance
  TH1D *nmEv3 = new TH1D("nmEv3","",200,0,40); //numu acceptance
  TH1D *neEv0 = new TH1D("neEv0","",200,0,40); //nue acceptance
  TH1D *neEv3 = new TH1D("neEv3","",200,0,40); //nue acceptance
...
/*
    if(LepPDG == 13){
      if(nu<10)  {nmEv0->Fill(Ev);}
      if(nu<0.3) {nmEv3->Fill(Ev);}}
    if(LepPDG == 11){
      if(nu<10)  {neEv0->Fill(Ev);}
      if(nu<0.3) {neEv3->Fill(Ev);}}
*/
...
/*   
    double s2mue2 = 0.0004;
    double s2mue2_1 = 0.01;
    double s2mm2, s2ee2;
    double dm2 = 4; //eV2
    double L = 500; //m
    double del = 1.27/1E3*dm2*L/Ev;
    double pmue = s2mue2 * pow(sin(del),2);
    double pmue_1 = s2mue2_1 * pow(sin(del),2);
    //double pmumu = 1 - pow(sin(2*tmumu),2) * pow(sin(del),2);  //mu disappearance   
    double pmumu = 1 - 2*sqrt(s2mue2) * pow(sin(del),2);  //mu disappearance   
    double pmumu_1 = 1 - 2*sqrt(s2mue2_1) * pow(sin(del),2);  //mu disappearance   
    //double pee = 1 - pow(sin(2*tee),2) * pow(sin(del),2);      //e disappearance
    double pee = 1 - 2*sqrt(s2mue2) * pow(sin(del),2);      //e disappearance
    double pee_1 = 1 - 2*sqrt(s2mue2_1) * pow(sin(del),2);      //e disappearance

    pmueVsEv0->Fill(Ev,pmue); pmumuVsEv0->Fill(Ev,pmumu); peeVsEv0->Fill(Ev,pmumu);
    pmueVsEv1->Fill(Ev,pmue_1); pmumuVsEv1->Fill(Ev,pmumu_1); peeVsEv1->Fill(Ev,pee_1);
*/
    if(LepPDG == 13){
      //if(nu<10)  {nmEv0->Fill(Ev);}
      //if(nu<0.3) {nmEv3->Fill(Ev);}
      ...
      if(reco_numu && (muon_contained || muon_tracker || muon_ecal)){
        if(nu<10)  {nmAcc0->Fill(Ev);}
        if(nu<0.3) {nmAcc3->Fill(Ev);}
        if(reco_nu<10)  {nmAcc0_1->Fill(Ev);}
        if(reco_nu<0.3) {nmAcc3_1->Fill(Ev);}
      }
    }
    if(LepPDG == 11){
      //if(nu<10)  {neEv0->Fill(Ev);}
      ...
      if(reco_nue){
        if(nu<10)  {neAcc0->Fill(Ev);}
        if(nu<0.3) {neAcc3->Fill(Ev);}
        if(reco_nu<10)  {neAcc0_1->Fill(Ev);}
        if(reco_nu<0.3) {neAcc3_1->Fill(Ev);}
      }
    }
  ...
  nmAcc0->Divide(nmEv0);
  nmAcc0->SetTitle("numu acceptance (fiducial && recontruction && true_nu / fiducial && true_nu)");
  nmAcc0->GetXaxis()->SetTitle("Ev (GeV)");
  //nmAcc0->GetYaxis()->SetTitle("reco_cut/true_cut");
  nmAcc3->Divide(nmEv3);
  neAcc0->Divide(neEv0);
  neAcc0->SetTitle("nue acceptance (fiducial && recontruction && true_nu / fiducial && true_nu)");
  neAcc0->GetXaxis()->SetTitle("Ev (GeV)");
  //neAcc0->GetYaxis()->SetTitle("reco_cut/true_cut");
  neAcc3->Divide(neEv3);
  nmAcc0_1->Divide(nmEv0);
  nmAcc0_1->SetTitle("numu acceptance (fiducial && recontruction && reco_nu / fiducial && true_nu)");
  nmAcc0_1->GetXaxis()->SetTitle("Ev (GeV)");
  //nmAcc0_1->GetYaxis()->SetTitle("reco_cut/true_cut");
  nmAcc3_1->Divide(nmEv3);
  neAcc0_1->Divide(neEv0);
  neAcc0_1->SetTitle("nue acceptance (fiducial && recontruction && reco_nu / fiducial && true_nu)");
  neAcc0_1->GetXaxis()->SetTitle("Ev (GeV)");
  //neAcc0_1->GetYaxis()->SetTitle("reco_cut/true_cut");
  neAcc3_1->Divide(neEv3);
